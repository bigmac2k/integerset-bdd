...
object Misc {
  ...
  def longBittedOp(normalop : (Long, Long) => Long, intlikeop : (IntLikeSet[Long, NBitLong], IntLikeSet[Long, NBitLong]) => IntLikeSet[Long, NBitLong]) : (Set[Long], Set[Long], Int) => Boolean =
      ...
      val ref = cartesianProduct(aBounded, bBounded).map((x) => NBitLong.signContract(bits_, normalop(x._1, x._2)))
